...
Agent 不是瓶颈，**状态才是**。大多数 Multi-Agent 系统的失败不是因为 Agent 不够聪明，而是因为没有单一事实源来管理：谁在改什么、哪些任务完成了、架构决策为什么这样做、当前项目是否健康。

Agent OS 定义了一个**协议 + 状态 + 调度**的三层架构：

```
协议层 (SKILL.md)    — 决策树、执行细则、验收标准、升级路径
状态层 (state.yaml)  — 11 模块的 Schema，所有 Agent 的共享世界模型
